...
int main(void) {
	...
	buf.mtype = 100;
	strcpy(buf.mtext,"info about buffer");
	msgsnd(msgqid,&buf,strlen(buf.mtext)+1,0);
	memset(buf.mtext,'\0',sizeof(buf.mtext));

	buf.mtype = 200;
	strcpy(buf.mtext,"text value none");
	msgsnd(msgqid,&buf,strlen(buf.mtext)+1,0);
	memset(buf.mtext,'\0',sizeof(buf.mtext));

	buf.mtype = 300;
	strcpy(buf.mtext,"from above");
	msgsnd(msgqid,&buf,strlen(buf.mtext)+1,0);
	memset(buf.mtext,'\0',sizeof(buf.mtext));
	...
	msgctl(msgqid,IPC_STAT,&ds); //with flag IPC_STAT copies info from stat struct
